...
globals [ev_times mouse-was-down? road-network shelter_locations shelter-locations evacuation-radius mortality_rate simulation_start_time patch_to_meter patch_to_feet fd_to_ftps fd_to_mph tick_to_sec min_lon min_lat max_evacuation_time max_evac_time]
...
to make-decision
  ...
  ifelse (rnd < 25) [ ; 25% have shorter milling time (prior knowledge or "klar | parat" app) and 75% have longer milling time
    set decision 1
...
to setup-init-val
  ...
  set max_speed 0.05  ; Increased max speed
...
to go
  ...
  let total-agents count residents
  if total-agents > 0 [
    set mortality_rate ((count residents with [dead?]) / total-agents) * 100
  ]
